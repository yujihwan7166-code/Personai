...
/**
 * AI 어시스턴트 음성 분석용 전사 엔드포인트.
 * - Whisper API `verbose_json` + timestamp_granularities=[segment] 로 타임스탬프 포함.
 * - study-transcribe 와는 별도 파일 (study 기능 리그레션 방지).
 */
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (!apiKey) {
    return res.status(501).json({
      error: '전사 서비스가 아직 연결되지 않았어요. 서버에 OPENAI_API_KEY를 설정해 주세요.',
    });
  }

  const { audioBase64, mimeType } = (req.body || {}) as ReqBody;
  if (!audioBase64) return res.status(400).json({ error: '오디오 데이터가 없어요.' });
