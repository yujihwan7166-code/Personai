  ...
  return (
    <section className="flex flex-col gap-2.5">
      <header className="flex items-baseline gap-2 px-1">
        <CalendarHeart className="h-3.5 w-3.5 text-amber-600 dark:text-amber-400" />
        ...
        {matches.map((entry) => {
          ...
          return (
            <button
              ...
              className={cn(
                'group flex items-start gap-3 rounded-xl border border-[hsl(var(--hairline))] bg-amber-50/30 dark:bg-amber-950/10 px-4 py-3 text-left',
                'hover:border-amber-500/30 hover:shadow-[0_2px_8px_-4px_hsl(45_85%_55%_/_0.2)] transition-all',
              )}
            >
              <span className="inline-flex items-center justify-center h-9 w-9 rounded-lg bg-amber-500/10 text-amber-700 dark:text-amber-300 shrink-0 text-[11px] font-bold tabular-nums">
                {ya}년 전
              </span>
              <div className="flex-1 min-w-0">
                ...
                <p className="font-serif text-[13.5px] text-foreground/90 leading-relaxed line-clamp-2">
                  {entry.body}
                </p>
              </div>
            </button>
