...
export function MainModeTabs({
  ...
  useEffect(() => {
    ...
    setAnchorLeft(Math.max(8, Math.min(Math.round(left), window.innerWidth - Math.min(1320, window.innerWidth - 32) - 8)));
  }, [open]);
      ...
      {typeof document !== 'undefined' && createPortal(
        ...
        {open && (
          <>
            ...
            {/* 좌측 270px 고정 + 콘텐츠 4칸 ~233px (카드 길이 복원) — 폭 배분 튜닝 2026-07-12 */}
            <div className="grid grid-cols-[270px_repeat(4,minmax(0,1fr))] grid-rows-[auto_1fr] gap-x-5 px-5 pt-5 pb-1">
