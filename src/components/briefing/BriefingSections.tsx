...
/** 레터 섹션 — 위 얇은 구분선 + 손글씨풍 소제목 + 내용. */
function LetterSection({ label, children }: { label: string; children: ReactNode }) {
  return (
...
export function HabitsSection({ data }: { data: BriefingData }) {
  ...
  return (
    <LetterSection label={`습관 · ${doneCount}/${data.habits.length}`}>
      <ul className="space-y-1.5">
