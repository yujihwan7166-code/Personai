      ...
      {selectedScenario && selectedScenario.simType === 'roleplay' && createPortal(
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4" onClick={() => setSelectedScenario(null)}>
          ...
          <div role="dialog" aria-modal="true" aria-labelledby="sim-step1-title" className="relative w-full max-w-[520px] max-h-[85vh] rounded-2xl bg-white shadow-2xl border border-slate-200 overflow-hidden flex flex-col animate-in fade-in zoom-in-95 duration-200"
            ...
            <div className={`shrink-0 h-1.5 bg-gradient-to-r ${selectedScenario.gradient}`} />
              ...
              <div className="flex items-center gap-3 pr-20">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  {selectedScenario.userRole && (
                    <span className="inline-flex items-center gap-1 mt-1.5 text-[10.5px] font-semibold px-2 py-0.5 rounded-full bg-white/80 text-indigo-700 border border-indigo-200/70">
                      🎭 당신은 <strong className="font-bold">{selectedScenario.userRole}</strong>
                    </span>
                  )}
                </div>
              </div>
            ...
            <div className="flex-1 overflow-y-auto px-5 py-4 space-y-5">
              ...
              <div>
                <div className="mb-2 flex items-center justify-between">
                  <span className="text-[11px] font-bold text-slate-700">참여 역할 · 총 {(selectedScenario.userRole ? 1 : 0) + selectedScenario.roles.length}명</span>
                  <span className="text-[10px] text-slate-400">AI가 자동 매칭</span>
                </div>
                <div className="space-y-1.5">
                  {/* 당신의 역할 */}
                  {selectedScenario.userRole && (
                    <div className={`relative flex items-center gap-2.5 px-3 py-2.5 rounded-xl border-2 border-indigo-300 bg-indigo-50/30 overflow-hidden min-h-[52px]`}>
                      <div className={`absolute left-0 top-0 bottom-0 w-1 bg-gradient-to-b ${selectedScenario.gradient}`} />
                      ...
                      <span className="text-[10px] font-bold text-indigo-500 px-2 py-0.5 rounded-full bg-indigo-100 shrink-0 tracking-wide">YOU</span>
                    </div>
                  )}
                  {selectedScenario.roles.map((role, ri) => {
                    ...
                    return (
                      <div key={role.name}>
                        <div className="flex items-center gap-2.5 px-3 py-2.5 rounded-xl border border-slate-200 bg-white min-h-[52px]">
                          <div className={`w-9 h-9 rounded-lg bg-gradient-to-br ${selectedScenario.gradient} flex items-center justify-center text-lg shrink-0 ring-1 ring-white/60`}>
                            {role.icon}
                          </div>
                          <div className="min-w-0 flex-1">
                            <div className="text-[11px] font-bold text-slate-700">{role.name}</div>
                            <div className="text-[10px] text-slate-400 line-clamp-1">{desc}</div>
                          </div>
                          <div className="flex gap-1 shrink-0">
                            {tags.map((tag, i) => (
                              <span key={i} className="text-[8.5px] px-1.5 py-0.5 rounded bg-indigo-50/60 text-indigo-600 font-medium">{tag}</span>
                            ))}
                          </div>
                        </div>
                      </div>
              ...
              <div>
                <div className="mb-2 flex items-center justify-between">
                  <span className="text-[11px] font-bold text-slate-700">리허설 흐름</span>
                  <span className="text-[10px] text-slate-400">단계별 진행</span>
                </div>
                <div className="space-y-0">
                  {(({
                    ...
                    <div key={i} className={cn(
                      ...
                      <span className={cn(
                        "w-6 h-6 rounded-lg bg-gradient-to-br text-[10px] font-black flex items-center justify-center shrink-0",
                        selectedScenario.gradient,
                        "text-slate-700/80 ring-1 ring-white/60"
                      )}>{i + 1}</span>
                      <span className={cn("text-[11.5px] leading-snug pt-0.5", i === arr.length - 1 ? "text-slate-800 font-semibold" : "text-slate-600")}>{text}</span>
                    </div>
                  ))}
                </div>
              </div>
            ...
            {/* Footer — 닫기 / 다음 */}
            <div className="shrink-0 px-5 py-3 border-t border-slate-100 bg-slate-50/70 backdrop-blur-sm flex items-center justify-between gap-3">
              <button
              ...
              <button
                ...
                className="px-4 py-2 rounded-xl bg-indigo-600 text-white text-[13px] font-bold hover:bg-indigo-700 transition-all flex items-center gap-1.5 shadow-[0_4px_14px_rgba(99,102,241,0.25)]"
              >
                다음 · 세부 설정 <ArrowRight className="w-4 h-4" />
              </button>
            </div>
          </div>
        </div>,
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4" onClick={() => setStep2Scenario(null)}>
          ...
          <div role="dialog" aria-modal="true" aria-labelledby="sim-step2-title" className="relative w-full max-w-[520px] rounded-2xl bg-white shadow-2xl border border-slate-200 overflow-hidden animate-in fade-in zoom-in-95 duration-200 max-h-[85vh] flex flex-col" onClick={e => e.stopPropagation()}>
            ...
            <div className="shrink-0 px-5 py-4 border-b border-slate-100 flex items-center gap-3">
              <div className={`w-10 h-10 rounded-xl bg-gradient-to-br ${step2Scenario.gradient} flex items-center justify-center text-[20px] ring-1 ring-white/60`}>
                {step2Scenario.icon}
              </div>
              <div className="flex-1 min-w-0">
            ...
            <div className="flex-1 overflow-y-auto px-5 py-4 space-y-5">
              ...
              {step2Scenario.prepQuestions.length > 0 && (() => {
                const total = step2Scenario.prepQuestions.length;
                const answered = step2Scenario.prepQuestions.filter(q => (step2Answers[q.id] || '').trim().length > 0).length;
                const allDone = answered === total;
                return (
                <div className="space-y-4">
                  <div className="flex items-center justify-between -mb-1">
                    <span className="text-[11px] font-bold text-slate-700">사전 정보 입력</span>
                    <span className={cn(
                      'text-[10px] font-medium',
                      allDone ? 'text-emerald-600' : 'text-slate-400'
                    )}>
                      {allDone ? '✓ 완료' : `완료 ${answered}/${total}`}
                    </span>
                  </div>
                    ...
                    <div key={q.id}>
                      ...
                      <div className="flex flex-wrap gap-1.5">
                        ...
                        <button
                          ...
                          className="px-3 py-1.5 rounded-full text-[11px] font-medium border border-dashed border-slate-300 text-slate-400 hover:border-indigo-300 hover:text-indigo-500 hover:bg-indigo-50/30 transition-all">
                          + 직접 입력
                        </button>
                      </div>
                      )}
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  <p className="text-[12px] font-bold text-slate-700">
                    ...
                    <span className="text-[10px] font-normal text-slate-400 ml-1.5">(선택 · 권장 1~3줄)</span>
                  </p>
                  <span className={cn(
                    'text-[10px] tabular-nums',
                    step2Context.length > 300 ? 'text-slate-400' : 'text-slate-300'
                  )}>{step2Context.length}자</span>
                </div>
                <textarea
                  value={step2Context}
              ...
              {/* 참고 자료 파일 업로드 */}
              <div>
                <p className="text-[12px] font-bold text-slate-700 mb-1.5">
                  참고 자료 첨부
                  <span className="text-[10px] font-normal text-slate-400 ml-1.5">선택 · 최대 {MAX_FILES}개 · PDF/DOCX/XLSX/이미지</span>
                </p>
                <input
                ...
                <button
                  ...
                  className="w-full inline-flex items-center gap-2.5 px-3 py-2.5 rounded-xl border border-dashed border-slate-300 text-[11px] font-medium text-slate-500 hover:border-indigo-300 hover:text-indigo-600 hover:bg-indigo-50/30 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  <span className="w-7 h-7 rounded-lg bg-indigo-50/60 flex items-center justify-center shrink-0">
                    {step2FileProcessing ? (
                  ...
                  <span className="text-[10px] text-slate-300">자소서·계약서·기획서</span>
                </button>
                {step2Files.length > 0 && (
                  <div className="mt-2 space-y-1">
                    {step2Files.map(f => (
                      <div key={f.id} className="flex items-center justify-between px-2.5 py-1.5 rounded-lg bg-white border border-slate-200">
                        <div className="flex items-center gap-2 min-w-0">
                        ...
                        <button
                          ...
                          className="text-slate-400 hover:text-red-500 shrink-0 ml-1.5 p-1 -m-1 rounded"
                          aria-label="파일 삭제"
                        >
                          <X className="w-3 h-3" />
                        </button>
                ...
                {step2FileError && (
                  <div className="mt-1.5 flex items-start gap-1.5 px-2 py-1 rounded-lg bg-red-50 border border-red-100">
                    <Info className="w-3 h-3 text-red-500 mt-0.5 shrink-0" />
                    <p className="text-[10px] text-red-600 font-medium leading-snug">{step2FileError}</p>
                  </div>
                )}
              </div>
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span className={cn('text-[10px] font-semibold tabular-nums',
                    settings.intensity <= 3 ? 'text-sky-600' : settings.intensity <= 6 ? 'text-indigo-600' : 'text-rose-500'
                  )}>{intensityLabel} · {settings.intensity}/10</span>
                </div>
                {/* 10단계 dot 인디케이터 */}
                <div className="flex items-center gap-1 mb-1.5 px-0.5">
                  {Array.from({ length: 10 }).map((_, i) => (
                    <span key={i} className={cn(
                      ...
                      i < settings.intensity ? 'bg-indigo-400' : 'bg-slate-200'
                    )} />
                  ))}
                </div>
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-[10px] text-rose-500 font-medium shrink-0">날카로운</span>
                </div>
              </div>
            </div>
            ...
            <div className="shrink-0 px-5 py-3 border-t border-slate-100 bg-slate-50/70 backdrop-blur-sm flex items-center justify-between gap-3">
              ...
              {(() => {
                const missing = step2Scenario.prepQuestions.length > 0 && step2Scenario.prepQuestions.some(q => !step2Answers[q.id]?.trim());
                const disabled = step2FileProcessing || missing;
                const title = step2FileProcessing
                  ? '파일 처리가 끝나면 시작할 수 있어요'
                  : missing
                    ? '모든 사전 질문에 답해주세요'
                    : '리허설을 시작합니다';
                return (
                  <button
                    ...
                    disabled={disabled}
                    title={title}
                    className="px-5 py-2.5 rounded-xl bg-indigo-600 text-white text-[13px] font-bold hover:bg-indigo-700 transition-all flex items-center gap-1.5 disabled:opacity-40 disabled:cursor-not-allowed shadow-[0_4px_14px_rgba(99,102,241,0.25)]"
                  >
                    리허설 시작 <ArrowRight className="w-4 h-4" />
                  </button>
                );
              })()}
            </div>
              ...
              {mainMode === 'general' ? 'GPT · Claude · Gemini — 원하는 AI 를 골라 자유롭게 대화'
                : mainMode === 'multi' ? '여러 AI의 답변을 비교하고 종합 결론으로'
                  : mainMode === 'debate' ? (
                    ...
                    : mainMode === 'stakeholder_main' ? `이해관계자 시점으로 의사결정 시뮬레이션 · ${SIMULATION_SCENARIOS.length}개 시나리오`
      ...
      {showExpertGrid && (
        <div className={cn(
          ...
          {effectiveCategory !== 'browser' && (searchMode && searchQuery.trim()
             ...
             *  ancano-pro 는 추천 탭에서는 노출되어야 하므로 추천 외에서만 숨김. */
