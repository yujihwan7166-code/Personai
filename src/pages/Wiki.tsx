 ...
 * 잉크 한 방울을 몇 %로 풀 것인가.
 *
 * 이 방의 선·옅은 면은 전부 같은 잉크(60,47,24)를 농도만 달리해 쓴다. 그런데 쓰다 보니
 * 농도가 18가지로 늘어 있었다 — .03 .04 .045 .05 .06 .07 .08 .09 .1 .12 .13 .14 …
 * 아무도 그 열여덟을 골라 쓴 게 아니라 그때그때 눈대중으로 적은 값이 쌓인 것이다.
 * 함수로 묶어 두면 어디에 몇 %가 쓰였는지 한눈에 세어지고, "선을 한 단계 진하게" 같은
 * 요청이 왔을 때 고칠 자리가 분명해진다.
 *
 * 쓰는 층 (이 사다리 밖의 값은 되도록 새로 만들지 않는다)
 *   .05  스치는 바탕 (hover)
 *   .07  눌린 바탕 (검색칸 · 세그먼트 트랙)
 *   .10  아주 옅은 칸선
 *   .14  기본 선
 *   .22  또렷한 선 (강조 · 테두리)
 */
const ink = (a: number) => `rgba(60,47,24,${a})`;

const C = {
  bg: '#f4eee1', paper: '#fdfaf2', ink: '#292217', body: '#332c21',
  sub: '#665c4b', muted: '#756a57',
  line: ink(0.14), line2: ink(0.09), lineDeep: ink(0.22),
  /** 스치는 바탕 · 눌린 바탕 — 여기저기 손으로 적던 값을 이름으로 모았다 */
  hover: ink(0.05), sunken: ink(0.07),
...
function DocMain({
  ...
  const infoboxSlot = active.infobox ? (
    ...
    <div className="relative z-10 mb-4 w-full sm:float-right sm:mb-3 sm:ml-8 sm:w-[360px]">
      <WikiInfoboxCard
