...
export type DebateSubMode = 'standard' | 'procon' | 'brainstorm' | 'redteam';
...
export const DEBATE_SUB_MODE_LABELS: Record<DebateSubMode, { label: string; icon: string; description: string }> = {
  ...
  redteam: { label: '레드팀 검증', icon: '🛡️', description: '공격팀이 허점을 찾고 방어팀이 보강' },
};

// Flat DiscussionMode for backward compat in logic
export type DiscussionMode = 'general' | 'multi' | 'expert' | 'standard' | 'procon' | 'brainstorm' | 'collaboration' | 'redteam' | 'assistant';
...
export const DISCUSSION_MODE_LABELS: Record<string, { label: string; icon: string; description: string; detail: string }> = {
  ...
  redteam: { label: '레드팀 검증', icon: '🛡️', description: '공격 vs 방어', detail: '공격팀이 허점을 찾고, 방어팀이 보강하여 아이디어를 검증합니다.' },
...
export interface DebateSettings {
  ...
  ideaCount: number;
  // 레드팀 검증 전용
  redteamIntensity: 'gentle' | 'standard' | 'ruthless';
  redteamFocus: 'logic' | 'feasibility' | 'risk' | 'all';
}
