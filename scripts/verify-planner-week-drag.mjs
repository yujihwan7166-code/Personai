...
async function verifyTodoToSchedulePromptKeyboard() {
  await withPlannerPage(async (page) => {
    ...
    await dialog2.getByRole('button', { name: '1시간 30분 길이 선택' }).focus();
    await page.keyboard.press('Enter');
    await page.waitForTimeout(250);
    assert(await dialog2.isVisible(), 'dialog closed after pressing Enter on duration preset');

    await dialog2.locator('input[type="time"]').fill('15:45');
    await page.keyboard.press('Enter');
