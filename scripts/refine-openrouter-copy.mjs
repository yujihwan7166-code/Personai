...
const templatePools = {
  ...
  coding: [
    ...
    ({ name, provider }) => `${name}: 저장소 이해와 개발 질의 응답을 작업 흐름으로 처리하기 좋은 ${provider} 모델`,
  ...
  vision: [
    ...
    ({ name, provider, context }) => `${name}: 시각 입력을 얹은 ${context} 업무 자료를 읽고 실행 항목으로 바꾸는 ${provider} 모델`,
    ({ name, provider, family }) => `${name}: ${family} 입력에서도 문서 화면과 대화 맥락을 함께 연결하는 ${provider} 모델`,
    ({ name, provider, price, family }) => `${name}: ${price} 조건의 ${family} 입력에서 표시된 화면 정보를 텍스트 판단으로 옮기는 ${provider} 모델`,
    ({ name, provider, context, family }) => `${name}: ${context} ${family} 입력을 바탕으로 이미지 순서와 문서 내용을 함께 정리하는 ${provider} 모델`,
  ],
  openWeight: [
    ({ name, provider, context }) => `${name}: ${context} 환경에서 오픈웨이트 실험과 비용 통제를 검토하기 좋은 ${provider} 모델`,
    ({ name, provider, price }) => `${name}: ${price} 운용을 고려해 자체 배포 후보와 공개 모델 비교에 맞춘 ${provider} 모델`,
    ({ name, provider, family }) => `${name}: ${family} 공개 모델로 서식, 평가, 로컬 이용 가능성을 살피기 좋은 ${provider} 모델`,
    ...
    ({ name, provider, price, family }) => `${name}: ${price} ${family} 이용을 염두에 둔 로컬 테스트와 모델 비교용 ${provider} 모델`,
  ],
  fast: [
    ...
    ({ name, provider, context }) => `${name}: ${context} 자료를 빠르게 읽고 실무용 초안을 만드는 ${provider} 모델`,
  ],
  general: [
